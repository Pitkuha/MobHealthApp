import { AiMessage, AiSession, ChatMessage, Lesson, ReferralCode, Reminder, User, WeekProgram } from './types';
...
export const api = {
  ...
  aiChat(token: string, message: string, sessionId?: string) {
    return request<{
      session: AiSession;
      answer: AiMessage;
      messages: AiMessage[];
  ...
  aiSessions(token: string) {
    return request<{
      sessions: Array<AiSession & { lastMessage?: string }>;
    }>('/ai/sessions', {}, token);
  },

  aiSessionMessages(token: string, sessionId: string) {
    return request<{
      session: AiSession;
      messages: AiMessage[];
