...
function buildCode(fullName) {
    const token = fullName.toUpperCase().replace(/[^A-ZА-Я]/g, '').slice(0, 4) || 'DOC';
    const random = Math.floor(1000 + Math.random() * 9000);
    return `${token}${random}`;
}
referralsRouter.post('/', requireAuth, requireRole(Role.DOCTOR), asyncHandler(async (req, res) => {
...
referralsRouter.patch('/:id', requireAuth, requireRole(Role.ADMIN), asyncHandler(async (req, res) => {
